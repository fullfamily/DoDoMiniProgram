// pages/order/shareCar/visitOrder/visitOrder.js
var app = getApp();
...
Page({
  ...
  data: {
    ...
    userOpenid: '',
    img_arr: [],
    departureTime: '',  // 比较特殊 因此需要做一下转换 额 上传的时候就把departure转换成String类型不就好了 
    // indicatorDots: true,
    // vertical: false,
    // autoplay: false,
    // circular: false,
  ...
  onLoad: function (options) {
    ...
    // var departureTime = util.date2String(date);
    var departureTime = orderDetail.departureTime;
    this.setData({
      orderOwnerOpenid: orderDetail._openid,
      departureTime: departureTime, // 纯属脱了裤子找屁放 不过在modify页面里 这种转换是用得到的
    })
    ...
    if (orderDetail.images_fileID.length != 0) {
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      title: 'DoDo好物',
      path: "/pages/order/shareCar/visitOrder/visitOrder?orderDetail=" + orderDetail
    }
  }
})